...
#model teacher
probability = pd.read_csv('label_probability.csv')#prediction result of hw3 with acc=96%
probability = np.array(probability.iloc[:,1:])
'''
Y_train_label = np.load('Y_train_label.npy')
ans = []
for i in range(len(probability)):
    ans.append(int(np.argmax(probability[i])))
for i in range(len(ans)):#accracy = 96%
    if ans[i]!= Y_train_label[i]:
        probability[i] = Y_train[i]
...
for i in range(len(Y_train_valid)):
    max_index = Y_train_valid[i].argmax(axis=0)
    for j in range(len(Y_train_valid[i])):
        if j == max_index:
            Y_train_valid[i][j] = 1
        else:
            Y_train_valid[i][j] = 0
