...
for j in range(epochs): 
    ...
    diff = x - x_adv
    temp = np.minimum(diff,dis)
    temp = np.maximum(temp,-dis)
    x_adv = x - temp
...
for i in range(pic_num):
	print(vgg19.decode_predictions(pre_preds, top=1)[i], vgg19.decode_predictions(preds, top=1)[i])

for i in range(start,start+pic_num):
	plot_img(i, x_adv[(i%pic_num)])

'''
Vgg-16 prediction attack rate: 0.795
Vgg-19 prediction attack rate: 0.905
ResNet-50 prediction attack rate: 0.525
ResNet-101 prediction attack rate: 0.465
DenseNet-121 prediction attack rate: 0.585
Densenet-169 prediction attack rate: 0.455
'''